    ...
    <div style={{ padding: 24 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24, flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ display: 'flex', gap: 12 }}>
          <select value={type} onChange={(e) => setType(e.target.value)} style={{ padding: '10px 16px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
            <option value="all">All Types</option>
            <option value="deposit">Deposits</option>
            <option value="withdrawal">Withdrawals</option>
          </select>
          <select value={filter} onChange={(e) => setFilter(e.target.value)} style={{ padding: '10px 16px', borderRadius: 8, border: '1px solid var(--border)', background: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: 16, marginBottom: 24 }}>
        ...
        <StatCard label="Pending" value={stats.pendingCount} color="#eab308" />
      </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 600 }}>
            <thead>
              <tr style={{ background: 'var(--bg-primary)' }}>
                ...
                <th style={{ textAlign: 'left', padding: 16, color: 'var(--text-secondary)', fontWeight: 500 }}>User</th>
            ...
            <tbody>
              {filteredTransactions.length === 0 ? (
                <tr>
                  <td colSpan={6} style={{ padding: 40, textAlign: 'center', color: 'var(--text-secondary)' }}>No transactions found</td>
                </tr>
              ) : (
                filteredTransactions.map(tx => (
                  <tr key={tx._id} style={{ borderBottom: '1px solid var(--border)' }}>
                    <td style={{ padding: 16, color: 'var(--text-secondary)' }}>{new Date(tx.createdAt).toLocaleDateString()}</td>
                    <td style={{ padding: 16, color: 'var(--text-primary)' }}>#{tx.oderId}</td>
                    <td style={{ padding: 16 }}>
                      <span style={{ padding: '4px 8px', borderRadius: 4, fontSize: 12, fontWeight: 600, background: tx.type === 'deposit' ? '#22c55e20' : '#ef444420', color: tx.type === 'deposit' ? '#22c55e' : '#ef4444' }}>
                        {tx.type?.toUpperCase()}
                      </span>
                    </td>
                    <td style={{ padding: 16, color: 'var(--text-primary)', fontWeight: 600 }}>₹{tx.amount?.toLocaleString()}</td>
                    <td style={{ padding: 16 }}>
                      <span style={{ padding: '4px 12px', borderRadius: 4, fontSize: 12, fontWeight: 600, background: tx.status === 'approved' ? '#22c55e' : tx.status === 'rejected' ? '#ef4444' : '#eab308', color: 'white' }}>
                    ...
                    <td style={{ padding: 16 }}>
                      {tx.status === 'pending' ? (
                        <div style={{ display: 'flex', gap: 8 }}>
                          <button onClick={() => processTransaction(tx._id, 'approved')} style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: '#22c55e', color: 'white', cursor: 'pointer', fontSize: 12 }}>Approve</button>
                          <button onClick={() => processTransaction(tx._id, 'rejected')} style={{ padding: '6px 12px', borderRadius: 6, border: 'none', background: '#ef4444', color: 'white', cursor: 'pointer', fontSize: 12 }}>Reject</button>
                        </div>
                      ) : (
