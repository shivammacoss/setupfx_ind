    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ background: 'var(--bg-secondary)', borderRadius: 12, padding: 24, marginTop: 24, border: '1px solid var(--border)' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <InfoRow label="Role" value={adminAuth?.user?.role?.replace('_', ' ').toUpperCase() || 'N/A'} />
          <InfoRow label="Status" value={adminAuth?.user?.isActive ? 'Active' : 'Inactive'} />
          <InfoRow label="Created" value={adminAuth?.user?.createdAt ? new Date(adminAuth.user.createdAt).toLocaleDateString() : 'N/A'} />
        </div>
      </div>
      ...
      <div style={{ background: 'var(--bg-secondary)', borderRadius: 12, padding: 24, marginTop: 24, border: '1px solid var(--border)' }}>
        ...
        <div style={{ marginBottom: 16, padding: 16, background: 'linear-gradient(135deg, #8b5cf6, #6366f1)', borderRadius: 12, textAlign: 'center' }}>
