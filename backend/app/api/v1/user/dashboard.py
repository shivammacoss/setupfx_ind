...
async def summary(user: CurrentUser):
    ...
    for h in holdings:
        try:
            ltp = await market_data_service.get_ltp(h.instrument.token)
            holdings_value += float(ltp) * float(h.quantity)
    ...
    for p in open_positions:
        try:
            ...
            today_pnl += (float(ltp) - float(to_decimal(p.avg_price))) * float(p.quantity)
