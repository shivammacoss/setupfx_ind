...
# Indian financial-news RSS feeds. Each entry is (name, url). Add more
# sources here as needed — the aggregator handles arbitrary fan-out.
...
async def _fetch_one(client: httpx.AsyncClient, source: str, url: str) -> list[dict[str, Any]]:
    try:
        r = await client.get(
            url,
            headers={"User-Agent": "Mozilla/5.0 (compatible; SetupFXBot/1.0)"},
